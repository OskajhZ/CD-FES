...
Year: 2025
Provides: 
    Computation of CD-AED (when __name__=="__main__")
    Definition of Common and Differential mode Frequency-domain Entropy Series (CD-FES), which will be used in main.py
...
def extract_AED(EEG_dir: str, save_dir: str) -> Tuple[tuple, tuple]:
    '''
    ...
    for idx, (subject, label) in tqdm(enumerate(EEG_dataset), total=len(EEG_labels)):
        ...
        if spectrums.mean() > 1e2:
            print(f"Bad Subject. Skip. Average={spectrums.mean()}, label={label}")
            excluded_count[int(label)] += 1
        else:
    ...
    print(f"Excluded count: positive={excluded_count[1]}, negative={excluded_count[0]}")
    pos_num = EEG_labels.sum()
    neg_num = len(EEG_labels) - pos_num
    print(f"Excluded ratio: positive={excluded_count[1]/pos_num}, negative={excluded_count[0]/neg_num}")
...
def process_spectrum(train_x, validate_x, to_FES=True):
    '''
...
class CrossValidationIter():
    ...
    def __init__(self, dataset_dir: str, fold_num = 10, data_type="CDFES"):
        '''
        data_type: "CDFES", "CDAED", "CDEEG"
        '''

        if data_type!="CDFES" and data_type!="CDAED" and data_type!="CDEEG":
    ...
    def clip(self, start_idx, end_idx):
        ...
        to_FES = True if self.data_type == "CDFES" else False
        train_set["data"], validate_set["data"] = process_spectrum(train_set["data"], validate_set["data"], to_FES)
...
if __name__ == "__main__":
    ...
    spectrum_dir = os.path.join(base_dir, "new_spectrum.h5")
    CDFES_dir = os.path.join(base_dir, "global_CDFES.h5")

    shapes = extract_AED(EEG_dir, spectrum_dir)
    print(shapes)
