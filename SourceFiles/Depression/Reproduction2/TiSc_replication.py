...
Year: 2025
Provides: 
    Reproduction of Time Scale Network
...
class EndToEndNetworkTrainer():
    ...
    def __init__(self, model: torch.nn.Module, train_loader: torch.utils.data.DataLoader, positive_ratio, validate_loader, validate_good_list, ensemble_amount = 3):
        ...
        input_samples, y = self.preprocess(input_samples, y)
    ...
    def preprocess(self, x, y):
        exponent = math.log(x.shape[-1], 2)
        if exponent-int(exponent) != 0:
            exponent = int(exponent) + 1
            zero_length = 2**exponent - x.shape[-1]
            zeros = torch.zeros([*x.shape[:-1], zero_length])
...
class IdentityMapping(torch.nn.Sigmoid):
    def __call__(self, x):
        return x
...
class TiScNet(torch.nn.Module):
    def __init__(self):
        super().__init__()
        get_tisc = lambda: torch.nn.Sequential(
                tiscLayer.TSC_input(2, 2**9),
                tiscLayer.TSC_hidden(2, 2**9, 2**10-1)
                )
        self.tisc_list = torch.nn.ModuleList(
                get_tisc() for _ in range(3)
                )
        self.mlp = torch.nn.Sequential(
                torch.nn.Linear(1533, 50),
                torch.nn.GELU(),
    ...
    def forward(self, x):
        ...
        for i in range(3):
            channel = x[:, i]
            tisc_y = self.tisc_list[i](channel)
            tisc_y_list.append(tisc_y)
        total_y = torch.cat(tisc_y_list, dim=-1)
        y = self.mlp(total_y)
        return y
