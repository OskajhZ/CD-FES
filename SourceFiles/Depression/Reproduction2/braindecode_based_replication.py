...
def cross_verification(spectrum_dir: str, ModelClass, **kwargs): # EEG contains in spectrum.h5
    '''
    ...
    for i, (train_set, validate_set) in enumerate(folds):
        ...
        chs_info = [
                {"ch_name": "Fp1", "kind": "eeg"},
                {"ch_name": "FpZ", "kind": "eeg"},
                {"ch_name": "Fp2", "kind": "eeg"},
                ]
        model = ModelClass(n_chans=3, n_outputs=2, input_window_seconds=3.0, sfreq=250.0, n_times=3*250, chs_info=chs_info)
