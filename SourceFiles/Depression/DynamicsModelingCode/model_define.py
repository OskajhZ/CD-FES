...
class EndToEndNetworkTrainer():
    ...
    def train(self, epochs = 10):
        ...
        for epoch in range(epochs):
            ...
            validate_result, ensemble_result, confusion_matrix, attention_score_dict = self.validate(False)
...
class BaselineDynamicModel(torch.nn.Module):

    def __init__(self, original_channels, state_dim, out_dim=2, model_type="LSTM"):
        super().__init__()
        self.model_type = model_type
        ModelClass = None
        if model_type == "LSTM":
            ModelClass = torch.nn.LSTM
        elif model_type == "GRU":
            ModelClass = torch.nn.GRU
        else:
            raise ValueError(f"Invalid model_type: {model_type}")
        
        self.dynamic = ModelClass(
                input_size = original_channels,
                hidden_size = state_dim,
                num_layers = 1,
                batch_first = True
                )
        self.norm = torch.nn.BatchNorm1d(state_dim)
        self.linear = torch.nn.Linear(state_dim, out_dim)

    def forward(self, x):
        '''
        x: [batch_size, channel, length]
